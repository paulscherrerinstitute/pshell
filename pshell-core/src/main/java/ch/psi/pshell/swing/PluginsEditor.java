...
public class PluginsEditor extends Editor {
    ...
    public static class PluginsEditorDocument extends Document {
        ...
        public void load(String fileName) throws IOException {
            clear();
            String pluginsConfigFile = Setup.getPluginsConfigurationFile();
    ...
    void create(Type type) {
        try {
            ...
            File file = Paths.get(Setup.getPluginsPath(), name + ".java").toFile();
            if (file.exists()) {
            ...
            if (jar == null) {
                if (Setup.isRunningInIde()) {
                    templatesFolder = Paths.get(Setup.getSourceAssemblyFolder(), "templates", "plugin").toFile();
                } else {
            ...
            IO.setFilePermissions(path, Context.getScriptFilePermissions());
            showMessage("Plugin Creation", "Success creating plugin: " + name);
            //Reload table
            ((PluginsEditorDocument) getDocument()).load(Setup.getPluginsConfigurationFile());
