...
public class MonitorScan extends LineScan {
    ...
    public StreamDevice getStream(){
        return (trigger instanceof StreamDevice streamDevice) ? streamDevice : null;
    }
        ...
        if (async){
            for (int i=0; i< originalReadables.length; i++){
                if (originalReadables[i] instanceof InlineDevice){
                    if (readables[i] instanceof ReadonlyRegisterBase reg){
                        reg.setAsyncUpdate(true);
                    }
                }
            }
        }
    ...
    static Device getSourceDevice(Readable r){
        ...
        if (r instanceof CacheReadable cacheReadable) {
        ...
        if (readables!=null){
            for (int i = 0; i < readables.length; i++) {
                if (async || (readables[i] == trigger)) {
                    if (readables[i] instanceof Cacheable cacheable){
                        readables[i] = cacheable.getCache();
                    }
                }
            }
        }
