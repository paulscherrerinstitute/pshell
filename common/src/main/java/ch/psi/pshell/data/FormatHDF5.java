...
/**
 * Format implementation storing data in HDF5 files.
 */
public class FormatHDF5 implements Format {
    ...
    public Path getFilePath(String path){
        return (writerFile==null) ? null : writerFile.toPath();
    }

    @Override
    public void openOutput(File root, boolean truncate) {
        writerFile = root;
        if (writerFile != null) {
            writerFile.mkdirs();
            if (truncate){
        ...
        try {
            if (writer != null) {          
                try {
                    ...
                    Logger.getLogger(FormatHDF5.class.getName()).log(Level.WARNING, null, ex);
                }
            }
    ...
    public DataSlice getData(String root, String path,  long[] index, int[] shape) throws IOException {        
        ...
        try {
            ...
            switch (dsinfo.getTypeInformation().getDataClass()) {
                case FLOAT:
                    array = switch (dsinfo.getTypeInformation().getElementSize()) {
                        case 4 -> reader.float32().readMDArrayBlock(path, shape, index).getAsFlatArray();
                        default -> reader.float64().readMDArrayBlock(path, shape, index).getAsFlatArray();
                    };
                    break;

                case INTEGER:
                    array = switch (dsinfo.getTypeInformation().getElementSize()) {
                        case 1 -> unsigned ? reader.uint8().readMDArrayBlock(path, shape, index).getAsFlatArray()
                                : reader.int8().readMDArrayBlock(path, shape, index).getAsFlatArray();
                        case 2 -> unsigned ? reader.uint16().readMDArrayBlock(path, shape, index).getAsFlatArray()
                                : reader.int16().readMDArrayBlock(path, shape, index).getAsFlatArray();
                        case 8 -> unsigned ? reader.uint64().readMDArrayBlock(path, shape, index).getAsFlatArray()
                                : reader.int64().readMDArrayBlock(path, shape, index).getAsFlatArray();
                        default -> unsigned ? reader.uint32().readMDArrayBlock(path, shape, index).getAsFlatArray()
    ...
    public DataSlice getData(String root, String path, int index) throws IOException {
        ...
        try {
            ...
            //Assume will read it all
            Object array =switch (dsinfo.getTypeInformation().getDataClass()) {
                case ENUM, STRING -> 
                    switch (rank) {
                        case 0 -> reader.readString(path);
                        default -> {
                            try{
                                yield reader.readStringArray(path);
                ...
                case FLOAT ->
                    switch (rank) {
                        case 0 -> reader.readDouble(path);
                        case 1 -> reader.readDoubleArray(path);
                        case 2 -> reader.readDoubleMatrix(path);
                        case 3 -> 
                            switch (dsinfo.getTypeInformation().getElementSize()) {
                                case 4 -> getMatrixArray (reader.float32().readMDArrayBlock(path, getMatrixShape(null), getMatrixOffset(index)));
                                default -> getMatrixArray (reader.float64().readMDArrayBlock(path, getMatrixShape(null), getMatrixOffset(index)));                            
                            };
                        default -> throw new IOException("Unsupported data rank: " + rank);
                    };                
                case INTEGER ->
                    switch (rank) {
                        case 0 ->   
                            switch (dsinfo.getTypeInformation().getElementSize()) {
                                case 1 -> unsigned ? reader.uint8().read(path) : reader.int8().read(path);
                                case 2 -> unsigned ? reader.uint16().read(path) : reader.int16().read(path);
                                case 8 -> unsigned ? reader.uint64().read(path) : reader.int64().read(path);
                                default -> unsigned ? reader.uint32().read(path) : reader.int32().read(path);
                            };                       
                        case 1 -> 
                            switch (dsinfo.getTypeInformation().getElementSize()) {
                                case 1 -> unsigned ? reader.uint8().readArray(path) : reader.int8().readArray(path);
                                case 2 -> unsigned ? reader.uint16().readArray(path) : reader.int16().readArray(path);
                                case 8 -> unsigned ? reader.uint64().readArray(path) : reader.int64().readArray(path);
                                default -> unsigned ? reader.uint32().readArray(path) : reader.int32().readArray(path);
                            };                       
                        case 2 ->
                            switch (dsinfo.getTypeInformation().getElementSize()) {
                                case 1 -> unsigned ? reader.uint8().readMatrix(path) : reader.int8().readMatrix(path);
                                case 2 -> unsigned ? reader.uint16().readMatrix(path) : reader.int16().readMatrix(path);
                                case 8 -> unsigned ? reader.uint64().readMatrix(path) : reader.int64().readMatrix(path);
                                default -> unsigned ? reader.uint32().readMatrix(path) : reader.int32().readMatrix(path);
                            };
                        case 3 -> 
                            switch (dsinfo.getTypeInformation().getElementSize()) {
                                case 1 -> getMatrixArray (unsigned ? reader.uint8().readMDArrayBlock(path, getMatrixShape(null), getMatrixOffset(index))
                                        : reader.int8().readMDArrayBlock(path, getMatrixShape(null), getMatrixOffset(index)));
                                case 2 -> getMatrixArray (unsigned ? reader.uint16().readMDArrayBlock(path, getMatrixShape(null), getMatrixOffset(index))
                                        : reader.int16().readMDArrayBlock(path, getMatrixShape(null), getMatrixOffset(index)));
                                case 8 -> getMatrixArray (unsigned ? reader.uint64().readMDArrayBlock(path, getMatrixShape(null), getMatrixOffset(index))
                                        : reader.int64().readMDArrayBlock(path, getMatrixShape(null), getMatrixOffset(index)));
                                default -> getMatrixArray (unsigned ? reader.uint32().readMDArrayBlock(path, getMatrixShape(null), getMatrixOffset(index))
                ...
                case BOOLEAN ->
                    switch (rank) {
                        case 0 ->  reader.readBoolean(path);
                        case 1 ->  Convert.fromByteArray(reader.uint8().readArray(path), boolean.class);
                        case 2 -> {
                ...
                case BITFIELD -> {
                    ...
                    for (int i = 0; i < b.length; i++) {
                        ((boolean[]) barray)[i] = b[i].get(0);
                    }
                    yield barray;
                }
                case COMPOUND -> 
                    reader.compound().readArray(path, getCompoundType(reader, path));                
                default -> 
                    null;
            };
            if (array != null) {
    ...
    Object getAttribute(IHDF5Reader reader, String path, String attr) {
        ...
        if (info.isArrayType()) {
            val = switch (info.getDataClass()) {
                case INTEGER -> switch (info.getElementSize()) {
                    case 1 -> unsigned ? reader.uint8().getArrayAttr(path, attr) : reader.int8().getArrayAttr(path, attr);
                    case 2 -> unsigned ? reader.uint16().getArrayAttr(path, attr) : reader.int16().getArrayAttr(path, attr);
                    case 8 -> unsigned ? reader.uint64().getArrayAttr(path, attr) : reader.int64().getArrayAttr(path, attr);
                    default -> unsigned ? reader.uint32().getArrayAttr(path, attr) : reader.int32().getArrayAttr(path, attr);
                };
                case FLOAT -> switch (info.getElementSize()) {
                    case 4 -> reader.float32().getArrayAttr(path, attr);
                    default -> reader.float64().getArrayAttr(path, attr);
                };
                default -> reader.string().getArrayAttr(path, attr);
            };
        } else {
            val = switch (info.getDataClass()) {
                case INTEGER -> switch (info.getElementSize()) {
                    case 1 -> unsigned ? reader.uint8().getAttr(path, attr) : reader.int8().getAttr(path, attr);
                    case 2 -> unsigned ? reader.uint16().getAttr(path, attr) : reader.int16().getAttr(path, attr);
                    case 4 -> unsigned ? reader.uint32().getAttr(path, attr) : reader.int32().getAttr(path, attr);
                    default -> unsigned ? reader.uint64().getAttr(path, attr) : reader.int64().getAttr(path, attr);
                };
                case FLOAT -> switch (info.getElementSize()) {
                    case 4 -> reader.float32().getAttr(path, attr);
                    default -> reader.float64().getAttr(path, attr);
                };
                case BOOLEAN -> reader.bool().getAttr(path, attr);
                default -> reader.string().getAttr(path, attr);
            };
        }
        return val;
    }
    ...
    byte getDeflation(Map features){
        ...
        if (compression != null){
            if (compression instanceof Number number){
    ...
    public void setItem(String path, Object data, Class type, int index) {       
        ...
        if (type == Double.class) {
            ...
            if (rank==2){
                writer.float64().writeMatrixBlockWithOffset(path, (double[][])data , index, 0);
            } else {            
            ...
            if (rank==2){
                writer.float32().writeMatrixBlockWithOffset(path, (float[][])data , index, 0);
            } else {
            ...
            if (rank==2){
                if (signed) {
                    writer.int64().writeMatrixBlockWithOffset(path, (long[][])data , index, 0);  
                } else {
                    writer.uint64().writeMatrixBlockWithOffset(path, (long[][])data , index, 0);
                }                                
            } else {                        
            ...
            if (rank==2){
                if (signed) {
                    writer.int32().writeMatrixBlockWithOffset(path, (int[][])data , index, 0);  
                } else {
                    writer.uint32().writeMatrixBlockWithOffset(path, (int[][])data , index, 0);
                }                                
            } else {                                    
            ...
            if (rank==2){
                if (signed) {
                    writer.int16().writeMatrixBlockWithOffset(path, (short[][])data , index, 0);  
                } else {
                    writer.uint16().writeMatrixBlockWithOffset(path, (short[][])data , index, 0);
                }                                
            } else {                                                
            ...
            if (rank==2){
                if (signed) {
                    writer.int8().writeMatrixBlockWithOffset(path, (byte[][])data , index, 0);  
                } else {
                    writer.int8().writeMatrixBlockWithOffset(path, (byte[][])data , index, 0);
                }                                
            } else {                                                
