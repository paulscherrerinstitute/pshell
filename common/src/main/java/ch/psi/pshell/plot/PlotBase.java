...
import ch.psi.pshell.swing.ImageTransferHandler;
...
abstract public class PlotBase<T extends PlotSeries> extends MonitoredPanel implements Plot<T>, Printable {
    ...
    protected void showDetached(PlotBase detachedPlot){
        detachedPlot.setPreferredSize(new Dimension(DETACHED_WIDTH, DETACHED_HEIGHT));
        
        Window window = getFrame();
        if (window==null){
            window=getWindow();
        }
        StandardDialog dlg = new StandardDialog(window, getTitle(), false);
        dlg.setCancelledOnEscape(false);
        dlg.setContentPane(detachedPlot);
        dlg.pack();
        SwingUtils.centerComponent(window, dlg);
        dlg.setVisible(true);
        dlg.requestFocus();
    }
    ...
    protected void notifyZoomListener(Range rangeX, Range rangeY){
        try{
            ...
            Logger.getLogger(PlotBase.class.getName()).log(Level.WARNING, null, ex);
        }        
    }    
