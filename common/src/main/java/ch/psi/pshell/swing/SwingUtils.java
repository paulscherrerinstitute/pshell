...
public class SwingUtils {
    ...
    public static void centerComponent(Component parent, Component component) {
        if ((Sys.getOSFamily() == OSFamily.Mac) && (component instanceof Window window)) {
    ...
    public static Rectangle getDefaultScreenUsableArea(){
        ...
        return new Rectangle(usableX, usableY, usableWidth, usableHeight);        
    }
        ...
        if (c!=null) {
            ...
            while ((c != null) && (c instanceof Window)) {
                if (c instanceof Frame frame) {
                    return frame;
                } else if (c instanceof Dialog dialog) {
                    c = dialog.getOwner();
                } else {
                    break;
                }
    ...
    public static void setFullScreen(final Frame frame, final boolean value) {
        final GraphicsDevice gd = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice();        
        if (value != isFullScreen(frame)) {
            boolean displayable = frame.isDisplayable();
            if (value) {
                if (displayable) {
                    frame.dispose();
                }
            ...
            if (Sys.getOSFamily() == Sys.OSFamily.Mac) {
                SwingUtilities.invokeLater(() -> {
                    //!!! TODO Setting to 50% is better than leaving unchanged, many splitters go to 100
                    for (JSplitPane splitPane: SwingUtils.getComponentsByType(frame, JSplitPane.class)){
    ...
    public static Component getComponentByName(Container parent, String name) {
        ...
        if (parent instanceof JMenu menu) {
            ...
            for (int i = 0; i < menu.getItemCount(); i++) {
                JMenuItem item = menu.getItem(i);
                if ((item != null) && (item instanceof JMenu m)) {
                    Component c = getComponentByName(m, name);
                    if (c != null) {
                        return c;
                    }
                }
    ...
    public static <T extends Component> T[] getComponentsByType(Container parent, Class<T> type) {
        ...
        for (int i = 0; i < componentCount; i++) {
            ...
            if (component instanceof Container container) {
                ret.addAll(Arrays.asList(getComponentsByType(container, type)));
            }
        }
    ...
    public enum OptionType {
        ...
        int toJOptionPaneType() {
            return switch (this) {
                case YesNo -> JOptionPane.YES_NO_OPTION;
                case YesNoCancel -> JOptionPane.YES_NO_CANCEL_OPTION;
                case OkCancel -> JOptionPane.OK_CANCEL_OPTION;
    ...
    public enum OptionResult {
        ...
        static OptionResult fromJOptionPaneResult(int result) {
            return switch (result) {
                case JOptionPane.YES_OPTION -> OptionResult.Yes;
                case JOptionPane.NO_OPTION -> OptionResult.No;
                case JOptionPane.CANCEL_OPTION -> OptionResult.Cancel;
                default -> OptionResult.Closed;
            }; //Equals OK
        }
    }
    ...
    public static JDialog showDialog(final Component parent, final String title, Dimension size, final JComponent content) {
        JDialog dialog = null;
        if (content.getTopLevelAncestor() instanceof JDialog dlg) {
            dialog = dlg;
        }
        ...
        if ((dialog == null) || (!dialog.isShowing())) {
            ...
            JPanel panel = null;
            if (content instanceof JPanel pn) {
                panel = pn;
            } else {
                panel = new JPanel();
                panel.setLayout(new BorderLayout());
            ...
            dialog.addWindowListener(new java.awt.event.WindowAdapter() {
                ...
                public void windowClosed(java.awt.event.WindowEvent evt) {
                    if (content instanceof AutoCloseable autoCloseable) {
                        try {
                            autoCloseable.close();
                        } catch (Exception ex) {
                        }
    ...
    public static JFrame showFrame(final Window parent, final String title, Dimension size, final JComponent content) {
        JFrame frame = null;
        if (content.getTopLevelAncestor() instanceof JFrame frm) {
            frame = frm;
        }
    ...
    public static Color readableStringToColor(String str) {
        if (str == null) {
            return null;
        }
    ...
    public static class CloseButtonTabComponent extends JPanel {
        ...
        private class TabButton extends JButton implements ActionListener {
            ...
            protected void paintComponent(Graphics g) {
                ...
                g2.setColor(isDark() ? Color.GRAY : Color.BLACK);
                if (getModel().isRollover()) {
                    g2.setColor(isDark() ? new Color(255, 40, 40) : new Color(180, 0, 0));
                }
                int delta = 6;
    ...
    public static boolean isDark() {
        if (dark==null){
            dark =  isDark(UIManager.getLookAndFeel().getClass());   
            SwingUtils.isDark();
        }
        return dark;            
    }    
    ...
    public static boolean isDark(String lafClassName) {
        if (lafClassName.contains("Dark") || lafClassName.contains("Darcula")){
            return true;
        }
        return false;
    }
    ...
    public static void setLookAndFeel(String className) {
        ...
        try {
            ...
            for (Window window : JFrame.getWindows()) {
                if (window instanceof StandardFrame standardFrame){
                    try {
                        standardFrame.onLafChange();
                    } catch (Exception ex) {
                    } 
                }                  
                if (window instanceof StandardDialog standardDialog){
                    try {
                        standardDialog.onLafChange();
                    } catch (Exception ex) {
                    } 
                }                  
    ...
    public static Color getColorError(){
        return isDark() ? new Color(255, 70, 70) :  Color.RED;
    }
    ...
    public static Color getColorRemote(){
        return isDark() ? Color.GRAY : Color.GRAY;
    }

    public static Color getColorStdout(){
        return isDark() ? new Color(187, 187, 187) :  Color.BLACK;
    }
    ...
    public static Color getColorStdin(){
        return isDark() ? new Color(0, 200, 0) : Color.GREEN;
    }    
