...
public class JythonUtils {
    ...
    public static String getPyMethodSignature(PyObject obj, String method, boolean includeJavaMethods) {
        try {
            ...
            if (f != null) {
                if (f instanceof PyMethod pm) {
                    return getPyMethodSignature(pm, includeJavaMethods);
                } else if (f instanceof PyFunction pf) {
                    return getFunctionSignature(pf, includeJavaMethods);
                } else if (f instanceof PyBuiltinCallable pc) {
                    return getPyBuiltinCallableSignature(method, pc);
                } else if (f instanceof PyReflectedFunction pr) {
                    return getFunctionSignature(pr, includeJavaMethods);
                }
            }
        } catch (Exception ex) {
        }
            ...
            try {
                if (f.getType().getClass() == PyType.class) {
                    ...
                    if (pars.size() > 0) {
                        ...
                        if ((def != null) && (def instanceof List defaults) && (((List) def).size() > 0)) {
    ...
    public static Map<String, Object> getFields(PyObject obj) {
        ...
        for (Object key : pymap.keys()) {
            ...
            if (val instanceof PyBoolean value) {
                val = value.getBooleanValue();
            } else if (val instanceof PyInteger value) {
                val = value.getValue();
            } else if (val instanceof PyFloat value) {
                val = value.getValue();
            } else {
