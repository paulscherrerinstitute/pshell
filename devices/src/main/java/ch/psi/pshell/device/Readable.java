...
public interface Readable<T> extends Record {
    ...
    default Class getElementType() {
        if (this instanceof ReadableType readableType) {
            return readableType._getElementType();
        }
        if (this instanceof CacheReadable cacheReadable) {
            Object obj = cacheReadable.getParent();
            if (obj instanceof ReadableType readableType) {
                return readableType._getElementType();
            }
        }        
        return Object.class;
    ...
    default Boolean isElementUnsigned() {
        if (this instanceof ReadableType readableType) {
            return readableType._isElementUnsigned();
        }
        return null;
    }
    ...
    default Class resolveElementType() throws IOException, InterruptedException {
        Class type = null;
        if (this instanceof Cacheable cacheable) {
            type = Arr.getComponentType(cacheable.take(Integer.MAX_VALUE));
        } else {
