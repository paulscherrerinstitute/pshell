...
public abstract class MotorBase extends PositionerBase implements Motor {
    ...
    final DeviceListener velocityChangeListener = new DeviceListener() {
        @Override
    ...
    protected void triggerSpeedChanged(Double value) {
        for (DeviceListener listener : getListeners()) {
            if (listener instanceof MotorListener motorListener) {
                try {
                    motorListener.onSpeedChanged(this, value);
                } catch (Exception ex) {
                    getLogger().log(Level.WARNING, null, ex);
                }
            }
    ...
    protected void triggerMotorStatusChanged(MotorStatus value) {
        for (DeviceListener listener : getListeners()) {
            if (listener instanceof MotorListener motorListener) {
                try {
                    motorListener.onMotorStatusChanged(this, value);
                } catch (Exception ex) {
                    getLogger().log(Level.WARNING, null, ex);
                }
            }
    ...
    protected Double doRead() throws IOException, InterruptedException {
        ...
        try {
            ...
            getLogger().log(Level.FINER, "Setpoint readout= {0}", readout);
            return readout;
        ...
        if (isSimulated()) {
            ...
            for (int retry = 1; retry <= retries; retry++) {
                try {
                    ...
                    Thread.sleep(1000);
                    getLogger().log(Level.INFO, "Error starting move: {0} - retry #{1}...", new Object[]{ex.getMessage(), retry});
                }
            }
        }
    }
    ...
    public MotorStatus readStatus() throws IOException, InterruptedException {
        ...
        try {
            ...
            currentStatus = ret;
            getLogger().log(Level.FINEST, "Status = {0}", ret);
            return ret;
        } catch (IOException ex) {
