...
public abstract class DeviceBase extends GenericDeviceBase<DeviceListener> implements Device {
    ...
    protected void setCache(DeviceBase child, Object value, Long timestamp, Long nanosOffset) {
        if (isChild(child)) {
            ...
            getLogger().log(Level.WARNING, "Attempt to set cache of not child device: {0}", child.getName());
        }
    }
    ...
    protected void addChild(Device child) {
        ...
        if (child != null) {
            if (!Arr.contains(children, child)) {
                ...
                if (child instanceof DeviceBase deviceBase) {
                    deviceBase.setParent(this);
                }
                if (trackChildren) {
    ...
    protected void setTrackChildren(boolean value) {
        if (trackChildren != value) {
            trackChildren = value;
            if (trackChildren) {
                trackedChildrenListener = new ReadbackDeviceListener() {
                    @Override
    ...
    public class DeviceException extends IOException {
        ...
        public String getMessage() {
            ...
            if ((getCause() == null) || (!(getCause() instanceof DeviceException deviceException)) || (deviceException.getDevice() != getDevice())) {
    ...
    protected void doClose() throws IOException {
        ...
        for (Device child : getChildren()) {
            try {
                ...
                getLogger().log(Level.FINE, "Error closing child device {0}: {1}", new Object[]{child.getName(), ex.getMessage()});
            }
        }
        cache = null;
        synchronized (valueWaitLock) {
        ...
        if (trackChildren) {
            ...
            for (Device child : getChildren()) {
                try {
                    ...
                    getLogger().log(Level.FINE, "Error updating child device {0}: {1}", new Object[]{child.getName(), ex.getMessage()});
                }
            }
        }
    }
    ...
    public void setTriggers(Device[] triggers) {
        ...
        for (Device trigger : getTriggers()) {
            if (triggerListener == null) {
                triggerListener = new DeviceListener() {
    ...
    protected void triggerReadbackChanged(Double value) {
        ...
        for (DeviceListener listener : getListeners()) {
            if (listener instanceof ReadbackDeviceListener readbackDeviceListener) {
                try {
                    readbackDeviceListener.onReadbackChanged(this, value);
                } catch (Exception ex) {
                    getLogger().log(Level.WARNING, null, ex);
                }
            }
