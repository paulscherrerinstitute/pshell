...
public class DeviceConfig extends Config {
    ...
    Map<String, Object> getJythonFieldDict() {
        if (Sys.hasJython()){
            if (this instanceof org.python.core.PyProxy pyProxy) {
                return JythonUtils.getFields(pyProxy._getPyInstance());
            }
        }
        return null;
    }
    ...
    public void copyFrom(Config config) {
        ...
        if (config instanceof DeviceConfig deviceConfig) {
            mapProperties = deviceConfig.mapProperties;
        }
    }
}
