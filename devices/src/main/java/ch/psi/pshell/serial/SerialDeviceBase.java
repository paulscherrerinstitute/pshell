...
public abstract class SerialDeviceBase extends DeviceBase implements SerialDevice {
    ...
    void log(String event, byte[] data) {
        ...
        if (logger.isLoggable(logLevel)) {
            logger.log(logLevel, "{0}: {1}", new Object[]{event, Convert.arrayToHexString(data, " ")});
        }
    }
    ...
    void log(String event, byte data) {
        Logger logger = getLogger();
        if (logger.isLoggable(logLevel)) {
            logger.log(logLevel, "{0}: {1}", new Object[]{event, Convert.arrayToHexString(new byte[]{data}, " ")});
        }
    }
    ...
    void log(String event, String data) {
        Logger logger = getLogger();
        if (logger.isLoggable(logLevel)) {
            logger.log(logLevel, "{0}: {1}", new Object[]{event, Convert.arrayToHexString(data.getBytes(), " ")});
        }
    }
