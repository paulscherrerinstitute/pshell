...
/**
 * Implements the Image/Stacks/Reslice command. Known shortcomings: for FREELINE or POLYLINE ROI,
 * spatial calibration is ignored: the image is sampled at constant _pixel_ increments (distance 1),
 * so (if y/x aspect ratio != 1 in source image) one dimension in the output is not homogeneous
 * (i.e. pixelWidth not the same everywhere).
 */
public class Slicer implements PlugIn, TextListener, ItemListener {
    ...
    //private static boolean rotastartAtSteS;
    //private static boolean flipS;
    //private static int sliceCountS = 1;

    public String startAt = starts[0];
    public boolean rotate;
    public boolean flip;
    public int sliceCount = 1;
    public boolean nointerpolate = Prefs.avoidResliceInterpolation;
    private double inputZSpacing = 1.0;
    public double outputZSpacing = 1.0;
    public int outputSlices = 1;
