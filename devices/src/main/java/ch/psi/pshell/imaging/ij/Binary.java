...
import ch.psi.pshell.imaging.ij.Binary;
import ij.*;
import ij.plugin.filter.*;
import ij.process.*;

/**
 * Implements the Erode, Dilate, Open, Close, Outline, Skeletonize and Fill Holes commands in the
 * Process/Binary submenu. Gabriel Landini contributed the clever binary fill algorithm that fills
 * holes in objects by filling the background. Version 2009-06-23 preview added, interations can be
 * aborted by escape (Michael Schmid)
 */
public class Binary implements PlugInFilter /*ExtendedPlugInFilter, DialogListener*/ {
    ...
    public int setup(String arg, ImagePlus imp) {
        ...
        if (doOptions) {
            if (imp == null) {
                return NO_IMAGE_REQUIRED;  //options dialog does not need a (suitable) image
            }
