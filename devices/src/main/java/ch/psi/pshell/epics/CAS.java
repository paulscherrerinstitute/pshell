...
public class CAS extends ProcessVariable implements AutoCloseable {
    ...
    public CAS(String channelName, ReadonlyRegisterBase register, String typeName) throws IOException, InterruptedException {
        ...
        register.addListener(new DeviceListener() {
            @Override
    ...
    public int getDimensionSize(int dimension) {
        ...
        return register instanceof ReadonlyRegisterArray readonlyRegisterArray ? readonlyRegisterArray.getSize() : 1;
    }
    ...
    public CAStatus write(DBR dbr, ProcessVariableWriteCallback callback) throws CAException {
        ...
        try {
            ...
            if (dbr instanceof STS sts) {
                sts.setSeverity(Severity.INVALID_ALARM);
                sts.setStatus(Status.WRITE_ALARM);
            }
    ...
    void obj2dbr(DBR dbr, Object object) throws Exception {
        if (dbr instanceof TIME time) {
            time.setTimeStamp(new TimeStamp());
        }
        if (object == null) {
            ...
            if (object.getClass().isArray()) {
                ...
                if (tbdr == String.class) {
                    ...
                    if (object instanceof Number number) {
                        Array.set(dbr.getValue(), 0, Convert.toType(number, tbdr));
                    } else {
        ...
        if (dbr instanceof PRECISION precision) {
            precision.setPrecision((short) register.getPrecision());
        }

        if (register instanceof ch.psi.pshell.device.ProcessVariable pv) {
            ...
            if (dbr instanceof GR gr) {
                ...
                if (!Double.isNaN(max)) {
                    gr.setUpperDispLimit(max);
                }
                if (!Double.isNaN(min)) {
                    gr.setLowerDispLimit(min);
                }
            }
            if (dbr instanceof CTRL ctrl) {
                if (!Double.isNaN(max)) {
                    ctrl.setUpperCtrlLimit(max);
                }
                if (!Double.isNaN(min)) {
                    ctrl.setLowerCtrlLimit(min);
                }
            }
        }
    }
