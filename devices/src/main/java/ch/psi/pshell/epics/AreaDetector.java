...
public class AreaDetector extends CameraBase {
    ...
    public AreaDetector(final String name, final String channelCtrl, final String channelData) {
        ...
        acquire.addListener(new DeviceListener() {
            @Override
        ...
        imageCounter.addListener(new DeviceListener() {
            @Override
            public void onValueChanged(Device device, Object value, Object former) {
        ...
        DeviceListener imageFormatListener = new DeviceListener() {
            @Override
            public void onValueChanged(Device device, Object value, Object former) {
                try {
                    ...
                    if (device == dataType) {
                        ...
                        if (isSimulated()){
                            ...
                            getLogger().log(Level.FINE, "Setting data type: {0} unsigned: {1}", new Object[]{type.getArrayType(), type.isUnsigned()});
    ...
    public void setTriggerMode(TriggerMode value) throws IOException, InterruptedException {
        ...
        if (value.isExternal()) {
            str = switch (getManufacturer()) {
                case "Prosilica" ->
                    switch (value) {
                        case External -> "Sync In 1";
                        case External_2 ->  "Sync In 2";
                        case External_3 ->  "Sync In 3";
                        case External_4 -> "Sync In 4";
                        default ->  value.toString();
                    };
                default -> TriggerMode.External.toString();
            };
        }
