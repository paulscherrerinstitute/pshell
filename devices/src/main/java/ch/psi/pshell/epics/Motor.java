...
import ch.psi.pshell.utils.BitMask;
...
public class Motor extends MotorBase {
    ...
    protected void doInitialize() throws IOException, InterruptedException {
        ...
        try {
            ...
            getLogger().log(Level.WARNING, "Cannot read version: {0}", ex.getMessage());
            version = 0.0;
        }
    }
    ...
    protected void doStartReferencing() throws IOException, InterruptedException {
        switch (getConfig().homingType) {
            case Backward -> startHoming(false);
            case Forward -> startHoming(true);
        }
    }
