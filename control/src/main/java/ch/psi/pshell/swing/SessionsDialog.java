...
public class SessionsDialog extends StandardDialog implements SessionManagerListener{
        ...
        try{
            ...
            for (int i=0; i<ids.size(); i++){
                try{
                    ...
                    String start = getDateTimeStr((Number)info.getOrDefault("start", 0));
                    String stop = getDateTimeStr((Number)info.getOrDefault("stop", 0));
    ...
    public class ScicatIngest extends Task {
        ...
        protected String doInBackground() throws Exception {
            ...
            try {
                ...
                JDialog splash = SwingUtils.showSplash(Context.getView(), "Archive", new Dimension(400,200), "Ingesting SciCat dataset...");
                try{
                ...
                SwingUtils.showScrollableMessage(Context.getView(), "SciCat Ingestion", msg, result.output);
                Logger.getLogger(SessionsDialog.class.getName()).info(msg + "\n" + result.output);
    ...
    public class ZipSession extends Task {
        ...
        protected String doInBackground() throws Exception {
            ...
            try {
                ...
                JDialog splash = SwingUtils.showSplash(Context.getView(),"Archive", new Dimension(400,200), "Generating ZIP file...");
                try{
                ...
                SwingUtils.showMessage(Context.getView(), "Archive", msg);
                Context.getApp().sendOutput(msg);
                setMessage(msg);
                setProgress(100);
                return msg;
    ...
    private void buttonAddFileActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buttonAddFileActionPerformed
        try {
            cancelMetadataEditing();
            JFileChooser chooser = new JFileChooser(Setup.getDataPath());
    ...
    private void buttonZIPActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buttonZIPActionPerformed
        try {
            ...
            if (rVal == JFileChooser.APPROVE_OPTION) {
                ...
                Context.getApp().startTask(new ZipSession(file));
            }
        } catch (Exception ex) {
            showException(ex);
        }     
    ...
    private void buttonScicatIngestionActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buttonScicatIngestionActionPerformed
        try {
            Context.getApp().getState().assertReady();
            Context.getApp().startTask(new ScicatIngest());
        } catch (Exception ex) {
            showException(ex);
            Logger.getLogger(SessionsDialog.class.getName()).log(Level.WARNING, null, ex);
        }
