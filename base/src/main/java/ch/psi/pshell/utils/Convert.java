...
public class Convert {
    ...
    public static Object toDouble(Object data) {
        ...
        if (data instanceof int[] is) {
            return intToDouble(is);
        } else if (data instanceof byte[] bs) {
            return byteToDouble(bs);
        } else if (data instanceof short[] ss) {
            return shortToDouble(ss);
        } else if (data instanceof long[] ls) {
            return longToDouble(ls);
        } else if (data instanceof BigInteger[] bis) {
            return bigIntToDouble(bis);
        } else if (data instanceof float[] fs) {
            return floatToDouble(fs);
        } else if (data instanceof boolean[] bs) {
            return booleanToDouble(bs);
        ...
        } else if (data instanceof int[][] array) {
            double[][] ret = new double[arrayLength][];
            for (int i = 0; i < arrayLength; i++) {
                ret[i] = intToDouble(array[i]);
            }
            ...
            return ret;
        } else if (data instanceof short[][] array) {
            double[][] ret = new double[arrayLength][];
            for (int i = 0; i < arrayLength; i++) {
                ret[i] = shortToDouble(array[i]);
            }
            ...
            return ret;
        } else if (data instanceof BigInteger[][] array) {
            double[][] ret = new double[arrayLength][];
            for (int i = 0; i < arrayLength; i++) {
                ret[i] = bigIntToDouble(array[i]);
            }
            return ret;
            ...
            return ret;
        } else if (data instanceof boolean[][] array) {
            double[][] ret = new double[arrayLength][];
            for (int i = 0; i < arrayLength; i++) {
                ret[i] = booleanToDouble(array[i]);
            }
            return ret;
        }
    ...
    public static Object toUnsigned(Object data) {
        ...
        if (data instanceof byte[][] bses) {
            short[][] ret = new short[Array.getLength(data)][];
            for (int i = 0; i < Array.getLength(data); i++) {
                ret[i] = Convert.toUnsigned(bses[i]);
            }
            return ret;
        }
        if (data instanceof short[][] sses) {
            int[][] ret = new int[Array.getLength(data)][];
            for (int i = 0; i < Array.getLength(data); i++) {
                ret[i] = Convert.toUnsigned(sses[i]);
            }
            return ret;
        }
        if (data instanceof int[][] ises) {
            long[][] ret = new long[Array.getLength(data)][];
            for (int i = 0; i < Array.getLength(data); i++) {
                ret[i] = Convert.toUnsigned(ises[i]);
            }
            return ret;
        }
        if (data instanceof long[][] lses) {
            BigInteger[][] ret = new BigInteger[Array.getLength(data)][];
            for (int i = 0; i < Array.getLength(data); i++) {
                ret[i] = Convert.toUnsigned(lses[i]);
            }
            return ret;
        }
        return data;
    ...
    public static Object toPrimitiveArray(Object wrapperArray) {
        Class listType = null;        
        if (wrapperArray instanceof List list) {   
    ...
    public static Object toWrapperArray(Object primitiveArray) {
        if (primitiveArray instanceof List list) {
    ...
    public static Object[][] to2dArray(Object[] arrayOfArrays) {
        ...
        for (Object array : arrayOfArrays) {
            if (array instanceof Object[] objects) {
                numCols = Math.max(numCols, objects.length);
            }
        }

        Object[][] twoDimArray = new Object[numRows][numCols];
        for (int i = 0; i < numRows; i++) {
            if (arrayOfArrays[i] instanceof Object[] array) {
    ...
    public static Object[] toArray(List list) {
        ...
        for (int i = 0; i < ret.length; i++) {
            if (ret[i] instanceof List l) {
                ret[i] = toArray(l);
            } else if (ret[i] instanceof Map map) {
                ret[i] = toArray(map);
            }
        }
        return ret;
