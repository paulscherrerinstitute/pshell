...
public class Utils {
    ...
    public static BufferedImage newImage(Object array, ImageDescriptor ip) {
        if ((ip.colorModel.getTransferType() == DataBuffer.TYPE_BYTE) && (array instanceof byte[] data)) {
            return Utils.newImage(data, ip);
        } else if ((ip.colorModel.getTransferType() == DataBuffer.TYPE_USHORT) && (array instanceof short[] data)) {
            return Utils.newImage(data, ip);
        }
    ...
    public static BufferedImage newImage(BufferedImage ref, Integer type, int width, int height) {
        ...
        BufferedImage ret;
        if ((cm instanceof IndexColorModel icm) && ((type == BufferedImage.TYPE_BYTE_INDEXED) || (type == BufferedImage.TYPE_BYTE_BINARY))) {
            ret = new BufferedImage(width, height, type, icm);
        } else {
