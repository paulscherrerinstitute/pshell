...
public class RedisClient {
    ...
    public void run(List<String> channels) {
        _logger.info("Starting Redis streaming: " + Str.toString(channels));
        try(Jedis jedis = new Jedis(host, port) ){       
            if (db!=null){
                jedis.select(db);
            }
            
            XReadParams params = XReadParams.xReadParams().count(1).block(10);            
            ...
            for (String channel : channels) {
                streamIds.put(channel, "$");  // New messages
            ...
            for (String channel : channels) {
                streams.add(new AbstractMap.SimpleEntry<>(channel.getBytes(), "$".getBytes()));
            }            
    ...
    public static void main(String[] args) {
        ...
        client.run(Arrays.asList("channel1", "channel2", "channel3"));
    }
}
