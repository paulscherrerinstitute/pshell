 ...
 */         
public class StreamValuePanel extends DevicePanel {
    ...
    @Override
    public Scalar getDevice() {
        return (Scalar) super.getDevice();
    }
    ...
    protected void onDeviceCacheChanged(Object value, Object former, long timestamp, boolean valueChange) {
            if (value!=null){
                ...
                if (value instanceof String){
                    ...
                    if (shape.length==1){
                          if (isImage){
                              double[][] data =  (double[][]) Convert.reshape((double[])Convert.toDouble(value), new int[]{((Long)height.take()).intValue(), ((Long)width.take()).intValue()});
    ...
    protected void createPlot(Object val){
        ...
        if (val!=null){
            ...
            if (val instanceof String){
                ...
                if (shape.length==1){
                    try{
                        if (getDevice().getName().equals("image")){
                            width = (Scalar) getDevice().getParent().getChild("width");
                            height = (Scalar) getDevice().getParent().getChild("height");
                    ...
                    //plot = new MatrixPlotJFree();
                    //plot.addSeries(new MatrixPlotSeries(getDevice().getName(), shape[0], shape[1]));
                    plot = new MatrixPlotRenderer();
                    plot.addSeries(new MatrixPlotSeries(getDevice().getName()));
                }
            } else {
