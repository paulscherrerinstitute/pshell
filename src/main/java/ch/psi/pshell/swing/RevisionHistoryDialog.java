 ...
 *
 */
public class RevisionHistoryDialog extends javax.swing.JDialog {

    final DefaultTableModel model;
    final String fileName;

    public RevisionHistoryDialog(java.awt.Frame parent, boolean modal, String fileName) throws Exception {
        super(parent, modal);
        initComponents();

        Context.getInstance().assertVersioningEnabled();
        this.fileName = IO.getRelativePath(fileName, Context.getInstance().getSetup().getHomePath());

        setTitle("Revision History - " + this.fileName);
        List<Revision> revisions = Context.getInstance().getVersioningManager().getHistory(this.fileName, 1000);
    ...
    void showRevision(String id) {
        try {
            diff.setText(Context.getInstance().getVersioningManager().getDiff(fileName, id));
            text.setText(Context.getInstance().getVersioningManager().fetch(fileName, id));
        } catch (Exception ex) {
            text.clear();
            diff.clear();
