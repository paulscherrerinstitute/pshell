...
/**
 * Complex actuator that consists of a list of other actuators. The steps of this actor are composed out of the steps of
 * the actors that this actuator consists of. First all the steps of the first actor are used, after that the steps of
 * the next actor are done. Before the first step of an actor pre actions are available and after the last step of an
 * actor a post action is available.
 */
public class ComplexActuator implements Actor {
    ...
    /**
     * Flag to indicate the first set() execution of this actor. This flag is used to decide whether to execute the pre
     * actions.
     */
