...
public class ImporterXscan implements Importer {
        ...
        for (DiscreteStepDimension dimension : scan.getDimension()) {
            for (DiscreteStepPositioner positioner : dimension.getPositioner()) {
                if (positioner instanceof FunctionPositioner) {
                    ...
                    FunctionPositioner fp = (FunctionPositioner) positioner;
                    add("");
            ...
            for (DiscreteStepPositioner positioner : dimension.getPositioner()) {
                ...
                if (!scanPositioners.contains(id)) {
                    ...
                    if (positioner instanceof LinearPositioner) {
                        LinearPositioner lp = (LinearPositioner) positioner;
                        ...
                        RegionPositioner rp = (RegionPositioner) positioner;
                        ...
                        PseudoPositioner pp = (PseudoPositioner) positioner;
                        scanStart.add("0.0");
                        ...
                        ArrayPositioner ap = (ArrayPositioner) positioner;
    ...
    void parseDetectors(Scan scan) {
        ...
        for (DiscreteStepDimension dimension : scan.getDimension()) {
            for (Detector detector : dimension.getDetector()) {
                ...
                if (detector instanceof ArrayDetector) {
    ...
    void parseManipulations(Scan scan) {
        ...
        for (Manipulation manipulation : scan.getManipulation()) {
            ...
            if (manipulation instanceof ScriptManipulation) {
                ScriptManipulation sm = (ScriptManipulation) manipulation;
    ...
    ArrayList<String> parseScript(List mappings, String script) {
        ...
        if (mappings.size() > 0) {
            ...
            for (Object obj : mappings) {
                if (obj instanceof ChannelParameterMapping) {
                    ChannelParameterMapping map = (ChannelParameterMapping) obj;
                    String type = map.getType();
                    ret.add(map.getVariable() + " = Channel('" + map.getChannel() + "', type = '" + getType(type) + "')");
                } else if (obj instanceof IDParameterMapping) {
                    IDParameterMapping map = (IDParameterMapping) obj;
                    ret.add(map.getVariable() + " =  " + resolveIdRef(map.getRefid()));
                } else if (obj instanceof VariableParameterMapping) {
                    VariableParameterMapping map = (VariableParameterMapping) obj;
    ...
    void addAction(Action action) {
        if (action instanceof ShellAction) {
            ShellAction sa = (ShellAction) action;
        ...
        } else if (action instanceof ChannelAction) {
            ...
            ScriptAction sa = (ScriptAction) action;
            add("#Script action");
    ...
    void openPositioner(DiscreteStepPositioner positioner) {
        ...
        } else if (positioner instanceof LinearPositioner) {
            LinearPositioner lp = (LinearPositioner) positioner;
    ...
    void addDetectors(List<Detector> detectors) {
        for (Detector detector : detectors) {
            ...
            if (detector instanceof SimpleDetector) {
                SimpleDetector sd = (SimpleDetector) detector;

                if (detector instanceof ScalerChannel) {
                    ScalerChannel sc = (ScalerChannel) detector;
                } else if (detector instanceof Timestamp) {
                    Timestamp t = (Timestamp) detector;
                    add(var + " = float(java.lang.System.currentTimeMillis())");
                    scanValues.add(var);
                    scanTags.put(id, var);
                } else if (detector instanceof SimpleScalarDetector) {
                    SimpleScalarDetector ssd = (SimpleScalarDetector) detector;
                }

            } else if (detector instanceof ComplexDetector) {
                ComplexDetector cd = (ComplexDetector) detector;
