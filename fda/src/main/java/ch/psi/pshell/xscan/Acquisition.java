...
public class Acquisition {
	...
	public Acquisition(ChannelService cservice, Config configuration, Map<String, Object> vars){
	...
	public void initalize(EventBus bus, Configuration smodel){            
		...
		try {
                        ...
                        Context.addDetachedFileToSession(xmlfile);
                        //if (!serializationTXT){
		...
		logger.fine("Map Model to internal logic");
			...
			// Setup optimized with manipulations
                        ...
                        EventBus b = new EventBus(Config.eventBusModeAcq);                        
...
//			col = collector;
		...
		if(actionLoop != null){
			...
			for(Channel<?> c: channels){
				try {
					...
					logger.log(Level.SEVERE, "Unable to destroy channel {0}: {1}", new Object[]{c.getName(), e.getMessage()});
				}
			}
			for(Object o: templates){
				try {
					...
					logger.log(Level.SEVERE, "Unable to destroy channels of template {0}: {1}", new Object[]{o.getClass().getName(), e.getMessage()});
				}
			}
	...
	public boolean canPause(){
            if (actionLoop instanceof ActorSensorLoop){
	...
	private static String resolveIdRef(Object object){
		...
		else if (object instanceof ch.psi.pshell.xscan.model.Manipulation manipulation){
				...
				for(ParameterMapping pm: sm.getMapping()){
					if(pm instanceof IDParameterMapping ipm){
						...
						if(cpm.getType().equals("String")){
							...
							logger.log(Level.WARNING, "Channel type [{0}] is not supported for mapping", cpm.getType());
						}
					}
					else if(pm instanceof VariableParameterMapping vp){						
	...
	private List<ch.psi.pshell.xscan.core.Action> mapActions(List<Action> actions){
		...
		for(Action a: actions){
			if(a instanceof ChannelAction ca){
				...
				if(operation.equals("put")){
					...
					if(type.equals("String")){
						...
						logger.log(Level.WARNING, "Operation {0} wity type {1} for action is not supported", new Object[]{operation, type});
					}
				}
					...
					logger.log(Level.WARNING, "Operation {0} for action is not supported", operation);
				}
			...
			else if(a instanceof ShellAction sa){
			...
			else if(a instanceof ScriptAction sa){				
				...
				for(ChannelParameterMapping ma: sa.getMapping()){
					if(ma.getType().equals("String")){
						...
						logger.log(Level.WARNING, "Channel type [{0}] is not supported for mapping", ma.getType());
					}
				}
        ...
        Device getDevice(String name){
            if (Context.hasDevicePool()){
                Device dev = Context.getDevicePool().getByName(name, Device.class);
                if (dev!=null){
                    return dev;
                }
            }
	...
	private ActorSensorLoop mapDiscreteStepDimension(DiscreteStepDimension dimension){
		...
		for(DiscreteStepPositioner p: dimension.getPositioner()){
			...
			if(p instanceof LinearPositioner lp){
                                Actor actuator;
                                Sensor sensor;
			...
			else if(p instanceof FunctionPositioner lp){
			...
			else if (p instanceof ArrayPositioner ap){
			...
			else if (p instanceof RegionPositioner rp){
			...
			else if(p instanceof PseudoPositioner pp){
				...
				// Not supported
				logger.log(Level.WARNING, "Mapping for {0} not available", p.getClass().getName());
			}
		}
	...
	private JythonFunction mapFunction(Function f){
		...
		for(ParameterMapping m: f.getMapping()){
			if(m instanceof VariableParameterMapping vp){
				Variable v = (Variable)vp.getName();
				JythonGlobalVariable var = jVariableDictionary.get(v.getName());
				var.setValue(v.getValue());
				map.put(vp.getVariable(), var);
                          
			} else if(m instanceof ChannelParameterMapping cpm){
                                JythonGlobalVariable var = new JythonGlobalVariable();
	...
	private void mapDetector(ActorSensorLoop aLoop, Detector detector){
		if(detector instanceof ScalarDetector sd){
			// Add pre actions
		...
		else if (detector instanceof DetectorOfDetectors dd){
			// Add pre actions
		...
		else if (detector instanceof Timestamp dd){
			// Ad sensor
			...
			// Not supported
			logger.log(Level.WARNING, "Detector type {0} not supported", detector.getClass().getName());
		}
	}
	...
	private ActionLoop mapContinuousDimension(ContinuousDimension dimension) {
		...
		CrlogicLoopStream actionLoop = new CrlogicLoopStream( cservice, zigZag, 
                        CrlogicConfig.getPrefix(), CrlogicConfig.getIoc(),  CrlogicConfig.getChannel(),  
                        CrlogicConfig.getAbortable(), CrlogicConfig.isSimulated());
	...
	private <T> Channel<T> createChannel(Class<T> type, String name, boolean monitor){
		...
		try {
			...
			throw new RuntimeException("Unable to create channel: "+name,e);
                        ...
                        logger.log(Level.WARNING, "Unable to create channel: {0} - Retrying", name);
                    }
		}
            }
            return null;
	...
	public static VDescriptor mapVisualizations(List<Visualization> vl){
		...
		for(Visualization v: vl){
			if(v instanceof ch.psi.pshell.xscan.model.LinePlot lp){				
			...
			else if(v instanceof ch.psi.pshell.xscan.model.LinePlotArray lp){
                                LinePlot lineplot = new LinePlot(lp.getTitle());
			...
			else if(v instanceof ch.psi.pshell.xscan.model.MatrixPlot mp){				
				...
				// X Axis
				if(mp.getX() instanceof LinearPositioner linp){
					idX = linp.getId();					
				...
				else if(mp.getX() instanceof PseudoPositioner pp){
					idX = pp.getId();
					minX = (1); // Count starts at 1
					maxX = (pp.getCounts());
					nX = (pp.getCounts());
				}
				else if(mp.getX() instanceof ContinuousPositioner conp){
					idX = conp.getId();					
				...
				// Y Axis
				if(mp.getY() instanceof LinearPositioner linp){
					idY = linp.getId();
				...
				else if(mp.getY() instanceof PseudoPositioner pp){
					idY = pp.getId();
			...
			else if(v instanceof ch.psi.pshell.xscan.model.MatrixPlotArray mp){
				// Support for 2D waveform plots
				// Get size of the array detector
				int arraySize = 0;
				Object o = mp.getZ();
				if(o instanceof ArrayDetector ad){
				...
				else if(mp.getY() instanceof ContinuousPositioner conp){
					idY = conp.getId();					
				...
				logger.log(Level.WARNING, "{0} is not supported as visualization type", v.getClass().getName());
			}
		}
		return vd;
	}
