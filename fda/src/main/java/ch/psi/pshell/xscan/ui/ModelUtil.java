...
public class ModelUtil {
    ...
    public void findInMappingAndRemove(Object object) {
        ...
        if (scan != null) {
            ...
            for (Manipulation m : scan.getManipulation()) {
                if (m instanceof ScriptManipulation sm) {
                    for (ParameterMapping p : sm.getMapping()) {
                        if (p instanceof IDParameterMapping pm) {
                            if (pm.getRefid().equals(object)) {
                                removeList.add(m);
                            }
                        }
                    }
            ...
            if (removeList.size() > 0) {
                 ...
                 */
                if (Context.getView().showOption("Warning", "There are manipulations refering to the deleted object. Do you want to delete these manipulations?", OptionType.YesNo) == OptionResult.Yes) {
                    // Remove manipulations from the model
                    scan.getManipulation().removeAll(removeList);
        ...
        for (Visualization v : configuration.getVisualization()) {
            ...
            } else if (v instanceof LinePlotArray lp) {
                if (lp.getY().size() > 0) {
                    // Check for references in Y
                    boolean found = false;
                    for (Object o : lp.getY()) {
            ...
            } else if (v instanceof MatrixPlot mp) {
            ...
            } else if (v instanceof MatrixPlotArray mp) {
        ...
        if (rlist.size() > 0) {
             ...
             */
            if (Context.getView().showOption("Warning", "There are visualizations refering to the deleted object. Do you want to delete these visualizations?", OptionType.YesNo) == OptionResult.Yes) {
                // Remove manipulations from the model
                configuration.getVisualization().removeAll(rlist);
    ...
    private void refreshLists(JPanel c) {
        for (Component co : c.getComponents()) {
            ...
            if (co instanceof ListContainer<?> lc) {
                lc.relayout();
            }

            if (co instanceof JPanel panel) {
                refreshLists(panel);
            }
        }
    }
    ...
    private void refreshIds(JPanel c) {
        for (Component co : c.getComponents()) {
            if (co instanceof LinePlotPanel panel) {
                panel.updateIds();
            }
            if (co instanceof MatrixPlotPanel panel) {
                panel.updateIds();
            }
            if (co instanceof IDParameterMappingPanel panel) {
                panel.updateIds();
            }
            if (co instanceof GlobalVariableParameterMappingPanel panel) {
                panel.updateIds();
            }
            if (co instanceof JPanel panel) {
                refreshIds(panel);
    ...
    private boolean wasModified(JPanel c) {
        ...
        if (c != null) {
            ...
            if (c instanceof EditableComponent lc) {
            ...
            for (Component co : c.getComponents()) {
//                if(co instanceof EditableComponent  lc){
//                    modified = lc.modified();
//                    if(modified){
