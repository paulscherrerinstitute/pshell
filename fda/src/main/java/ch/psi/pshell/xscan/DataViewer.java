...
public final class DataViewer extends MonitoredPanel {
        ...
        menuBrowse.addActionListener((ActionEvent e) -> {
            try {
                ...
                if ((file != null) && (file.exists())) {
                    Logger.getLogger(DataViewer.class.getName()).fine("Opening desktop for: " + String.valueOf(file));
        ...
        if ((file != null) && (file.exists() && (file.isFile()))) {
            try {
                switch (IO.getExtension(file)) {
                    case "log", "xml", "h5" -> openFile(file);
                    case "txt" -> plotFile(file);
                }
    ...
    void openFile(File file) throws Exception {
        if ((file != null) && (file.exists() && (file.isFile()))) {
            ...
            switch (IO.getExtension(file)) {
                case "log":
                case "mat":
                    Editor editor = Context.getView().openTextFile(filename);
                    editor.setReadOnly(true);
                    break;
                case "xml":
                    Context.getView().openScriptOrProcessor(filename);
                    break;
                case "h5":
                    for (DataPanel pn : Context.getView().getDataFilePanels()) {
                        if (file.equals(new File(pn.getFileName()))) {
                            if (Context.getView().getDocumentsTab().indexOfComponent(pn) >= 0) {
                                Context.getView().getDocumentsTab().setSelectedComponent(pn);
                            } else if (Context.getView().getDetachedComponents().containsValue(pn)) {
                    ...
                    Context.getView().openDataFile(filename.toString());
                    break;
                case "txt":
                    Editor ed = Context.getView().openTextFile(filename);
                    ed.setReadOnly(true);
            }
        }
    }
    ...
    class FileSystemModel implements TreeModel {
        ...
        public File[] getChildren(Object parent) {
            ...
            if (f.isDirectory()) {
                ...
                switch (fileOrder) {
                    case Modified -> IO.orderByModified(subfolders);
                    case Name -> IO.orderByName(subfolders);
                }
                ...
                switch (fileOrder) {
                    case Modified -> IO.orderByModified(files);
                    case Name -> IO.orderByName(files);
                }
                for (File file : files) {
    ...
    DefaultMutableTreeNode createTreeNode(Object[] data) {
        ...
        for (int i = 1; i < data.length; i++) {
            ...
            if (nodeSpecifier instanceof Object[] oa) {
                child = createTreeNode(oa);
            } else {
