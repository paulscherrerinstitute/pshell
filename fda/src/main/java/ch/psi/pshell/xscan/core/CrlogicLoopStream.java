...
public class CrlogicLoopStream implements ActionLoop {
    ...
    private void simulate() throws IOException, TimeoutException, InterruptedException{
        ...
        // Move to start
        logger.log(Level.INFO, "Move motor to start [{0}]", start);
        ...
        logger.log(Level.INFO, "Move motor to end [{0}]", end);
        try {
        ...
        try {
            ...
            if (!template.getStatus().getValue().equals(TemplateCrlogic.Status.INACTIVE.toString())) {
                ...
                if (template.getStatus().getValue().equals(TemplateCrlogic.Status.FAULT.toString())) {
                    ...
                    logger.log(Level.INFO, "Error message: {0}", template.getMessage().getValue());
            ...
            logger.log(Level.INFO, "Ticks per second: {0}", tps);
            ...
            logger.log(Level.INFO, "Estimated time: {0}:{1}:{2}", new Object[]{hours, minutes, seconds});
            ...
            if (integrationTime < minIntegrationTime) {
                ...
                logger.log(Level.INFO, "Integration time is too small [min integration time: {0}]", minIntegrationTime);
...
// Start crlogic logic
                ...
                try {
                    ...
                    logger.log(Level.INFO, "Failed to start CRLOGIC. Logic in status: {0}", template.getStatus().getValue());
                    if (template.getStatus().getValue().equals(TemplateCrlogic.Status.FAULT.toString())) {
                        logger.log(Level.INFO, "Error message: {0}", template.getMessage().getValue());
                    }
                    // Recover to inactive
...
//				drainHangingSubmessages();
                ...
                // Move motor(s) to end / wait until motor is stopped
                logger.log(Level.INFO, "Move motor to end [{0}]", realEnd);
                try {
                    ...
                    try{
                        ...
                        // Send end of stream message
                        logger.log(Level.INFO, "Sending - End of Line - Data Group: {0}", dataGroup);
                        eventbus.post(new EndOfStreamMessage(dataGroup));
                    } finally{
                ...
                try {
                    ...
                    logger.log(Level.INFO, "Failed to stop CRLOGIC. Logic in status: {0}", template.getStatus().getValue());
                    // TODO Improve error handling
    ...
    public void prepare() {
        ...
        try {
            ...
            logger.log(Level.INFO, "Motor type: {0}", TemplateMotor.Type.values()[motortemplate.getType().getValue()]);
            logger.log(Level.INFO, "Motor use readback: {0}", useReadback);
            logger.log(Level.INFO, "Motor use encoder: {0}", useEncoder);
            ...
            if ((!useReadback) && (!useEncoder)) {
                ...
                try{
                    ...
                    logger.log(Level.WARNING, "Error reading encoder fields: {0}", ex.getMessage());
                } finally{
                    try{
    ...
    private void connect() {
        ...
        if (useIoc()){
            logger.log(Level.INFO, "Connecting with IOC{0}", ioc);
            context = ZMQ.context(1);
            socket = context.socket(ZMQ.PULL);
            socket.setRcvHWM(HIGH_WATER_MARK);
            socket.connect("tcp://" + ioc + ":9999");
        } else if (channel  != null){
            logger.log(Level.INFO, "Connecting to channel: {0}", channel);
    ...
    private void close() {
        ...
        try {
            if (useIoc()){
                logger.log(Level.INFO, "Closing stream from IOC {0}", ioc);
                socket.close();
                context.close();
            } else if (channel!=null){
                logger.log(Level.INFO, "Closing stream from channel {0}", channel);
                dataChannel.close();
            }
        } catch (Exception ex) {
